...
def new_learner(id, name, is_man):
    cursor_learner.execute(f"INSERT INTO learner VALUES ({id},'{name}',{is_man},{randint(-100, 100)},{randint(0, 10)},{randint(0, 10)},{randint(0, 10)},0)")
    conn.commit()
...
def money_learner(name, money):
    ...
    sql = f"UPDATE learner SET hod=3 WHERE name='{name}'"
    cursor_learner.execute(sql)
    conn.commit()
...
def learner_study(name, lessen):
    ...
    if check[0] <= 30:
        sql = f"UPDATE learner SET {lessen}={check[0]+randint(0,3)} WHERE name='{name}'"
        cursor_learner.execute(sql)
        conn.commit()
        sql = f"SELECT {lessen} FROM learner WHERE name='{name}'"
        cursor_learner.execute(sql)
...
def check_learner(id, list_of_name):
    sql = "SELECT * FROM learner WHERE id=?"
    cursor_learner.execute(sql, [id])
    check = cursor_learner.fetchone()
    if check is None:
        gender = randint(0, 1)
        new_learner(id, new_name(list_of_name, gender), gender)
        sql = "SELECT * FROM learner WHERE id=?"
        cursor_learner.execute(sql, [id])
        check = cursor_learner.fetchone()
    return check
